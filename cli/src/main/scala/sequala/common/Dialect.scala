...
object OracleDialect extends Dialect with sequala.schema.oracle.OracleDialect {
  ...
  def renderTables(tables: Seq[DialectTable], pretty: Boolean): String =
    ...
    // Render CREATE INDEX statements for non-constraint indexes
    val indexStatements = tables.flatMap { t =>
      ...
      t.indexes.flatMap { idx =>
        idx.name.map { idxName =>
          val qualifiedIndexName = s"""$schemaPrefix"$idxName""""
    ...
    (createStatements ++ indexStatements ++ commentStatements).mkString(";\n\n") + ";"
