...
case class SqlFormatConfig(pretty: Boolean = false, indent: String = "  "):
  private val logger = Logger.of[SqlFormatConfig]
  ...
  def renderLeadingComments(comments: Seq[SqlComment]): String =
    logger.debug(s"renderLeadingComments: ${comments.size} comments, pretty=$pretty")
    if comments.nonEmpty then logger.debug(s"renderLeadingComments: comments=${comments.map(_.text).mkString("; ")}")
    val result =
      if comments.isEmpty then ""
      else if pretty then comments.map(c => renderComment(c)).mkString("", s"\n$indent", "\n" + indent)
      else comments.map(c => renderComment(c)).mkString("", "\n", "\n")
    logger.debug(s"renderLeadingComments: result='${result.take(80).replace("\n", "\\n")}${
        if result.length > 80 then "..." else ""
      }'")
    result
