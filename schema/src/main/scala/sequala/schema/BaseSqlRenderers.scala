...
trait BaseSqlRenderers extends LogSupport:
  ...
  given columnRenderer[DT <: DataType, CO <: ColumnOptions]: SqlRenderer[Column[DT, CO]] with
    private val rendererLogger = Logger.of[BaseSqlRenderers]
    def toSql(col: Column[DT, CO])(using config: SqlFormatConfig): String =
      ...
      rendererLogger.debug(
        s"columnRenderer: '${col.name}' has ${col.sourceComment.size} sourceComments, pretty=${config.pretty}"
      )
      if col.sourceComment.nonEmpty then
        rendererLogger.debug(s"columnRenderer: '${col.name}' comments: ${col.sourceComment.map(_.text).mkString("; ")}")
      ...
      rendererLogger.debug(
        s"columnRenderer: '${col.name}' leadingComments='$leadingComments', trailingComments='$trailingComments'"
      )
  ...
  given createTableRenderer[DT <: DataType, CO <: ColumnOptions, TO <: TableOptions](using
    ...
    def toSql(ct: CreateTable[DT, CO, TO])(using config: SqlFormatConfig): String =
      ...
      rendererLogger.debug(
        s"createTableRenderer: '${ct.table.name}' has ${tableComments.size} sourceComments, pretty=${config.pretty}"
      )
      if tableComments.nonEmpty then
        rendererLogger.debug(
          s"createTableRenderer: '${ct.table.name}' comments: ${tableComments.map(_.text).mkString("; ")}"
        )
      val leadingComments =
      ...
      rendererLogger.debug(
        s"createTableRenderer: '${ct.table.name}' leadingComments='${leadingComments.take(50)}...', trailingComments='$trailingComments'"
      )
