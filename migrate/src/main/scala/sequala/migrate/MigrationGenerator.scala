...
trait MigrationGenerator[DT <: DataType, CO <: ColumnOptions, TO <: TableOptions]:
  def generate(changes: Seq[SchemaDiff[DT, CO, TO]]): Seq[MigrationStep[DT, CO, TO]] =
    changes.map(toMigrationStep)

  def toMigrationStep(change: SchemaDiff[DT, CO, TO]): MigrationStep[DT, CO, TO] =
    change match
  ...
  private def describeAction(action: AlterTableAction[DT, CO]): String =
    action match
      case AddColumn(col) => s"add column ${col.name}"
      case DropColumn(name, _) => s"drop column $name"
      case ModifyColumn(col) => s"modify column ${col.name}"
      case RenameColumn(oldName, newName) => s"rename column $oldName to $newName"
      case AddConstraint(constraint) =>
        constraint match
      ...
      case DropConstraint(name, _) => s"drop constraint $name"
