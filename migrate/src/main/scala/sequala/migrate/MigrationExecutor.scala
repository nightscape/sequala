...
trait SchemaDiffRenderer[DT <: DataType, CO <: ColumnOptions, TO <: TableOptions] {
  ...
  def render(diff: SchemaDiff[DT, CO, TO]): Seq[String] = diff match {
          ...
          failedStep match {
            case Some(_) if rollbackOnFailure =>
              ...
              val rolledBackResults = results.map { r =>
                if (r.status == StepStatus.Success) r.copy(status = StepStatus.RolledBack)
                else r
              }
              val endNanos = System.nanoTime()
