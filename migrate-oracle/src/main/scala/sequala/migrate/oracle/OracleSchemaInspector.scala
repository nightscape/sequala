...
object OracleSchemaInspector extends SchemaInspector[OracleDataType, OracleColumnOptions, OracleTableOptions]:
  ...
  def inspectTable(
    ...
    if columns.isEmpty then None
      ...
      Some(Table(tableName, columns, primaryKey, indexes, foreignKeys, checks, uniques, tableOptions))
  ...
  private def getColumns(
    ...
    while rs.next() do
      ...
      val dataType = parseDataType(dataTypeName, dataLength, dataPrecision, dataScale, charUsed)
  ...
  private def parseDataType(
    ...
    typeName.toUpperCase match
      ...
      case t if t.startsWith("TIMESTAMP") =>
        ...
        SqlTimestamp(dataPrecision.orElse(Some(6)), withTz)
