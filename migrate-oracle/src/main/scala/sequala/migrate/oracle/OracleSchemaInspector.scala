...
object OracleSchemaInspector extends SchemaInspector[OracleDataType, OracleColumnOptions, OracleTableOptions]:
  ...
  private def getColumns(
    ...
    while rs.next() do
      ...
      // Filter out internal identity sequence defaults (ISEQ$$_nnn) - these are Oracle-generated
      // for GENERATED AS IDENTITY columns and should not be exported
      val actualDefault =
        if virtualColumn == "YES" then None
        else default.filterNot(_.contains("ISEQ$$"))
