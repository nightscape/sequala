...
class OracleSQL extends SQLBase {
  ...
  def oracleTableField[$ : P]: P[Either[
    ...
    (
      keyword("CONSTRAINT") ~
        identifier ~
        (
          (keyword("PRIMARY") ~ keyword("KEY") ~ oneOrMoreAttributes).map(cols =>
            ("pk", cols, None: Option[Expression])
          ) |
            (keyword("UNIQUE") ~ oneOrMoreAttributes).map(cols => ("unique", cols, None: Option[Expression])) |
            (keyword("CHECK") ~ "(" ~/ expression ~ ")").map(expr => ("check", Seq.empty[Name], Some(expr)))
        )
    ).map { case (constraintName, (kind, cols, exprOpt)) =>
      kind match
        case "pk" => Left(TablePrimaryKey(cols, Some(constraintName)))
        case "unique" => Left(TableUnique(cols, Some(constraintName)))
        case "check" => Left(TableCheck(Some(constraintName), exprOpt.get))
    } |
  ...
  def oracleCreateTablePositioned[$ : P]: P[PositionedCreateTable] = P(
    (
      ...
      case (tablePos, orReplace, (schemaOpt, tableName), Right(positionedFields), clausesOpt) =>
        ...
        val checks = annotations.collect { case tc: TableCheck =>
          import sequala.common.renderer.ParserSqlRenderers.given
          import sequala.schema.{SqlFormatConfig, SqlRenderer}
          given SqlFormatConfig = sequala.schema.SqlFormatConfig.Compact
          Check(tc.name.map(_.name), summon[SqlRenderer[Expression]].toSql(tc.expression))
        }
  ...
  private def convertOracleTableClausesToOptions(
    ...
    sequala.schema.oracle.OracleTableOptions(
      ...
      compress = clauses.compress.flatMap {
        case "BASIC" => Some(sequala.schema.oracle.OracleCompressBasic)
        case "ADVANCED" => Some(sequala.schema.oracle.OracleCompressAdvanced)
        case "NONE" => None // NOCOMPRESS - no compression
        case other => Some(sequala.schema.oracle.OracleCompressFor(other))
      },
      cache = clauses.cache,
  ...
  def compressClause[$ : P]: P[String] = P(
    (StringInIgnoreCase("COMPRESS") ~/ (
      StringInIgnoreCase("BASIC").map(_ => "BASIC") |
        ...
        // Handle two-word compression types like "QUERY HIGH", "ARCHIVE LOW"
        (StringInIgnoreCase("FOR") ~/ (identifier ~ identifier.?).map {
          case (first, Some(second)) => s"${first.name.toUpperCase} ${second.name.toUpperCase}"
          case (first, None) => first.name.toUpperCase
        })
    ).?).map {
  ...
  private def oracleStringTypes[$ : P]: P[sequala.schema.oracle.OracleDataType] = P(
    (StringInIgnoreCase("VARCHAR2") ~ "(" ~ integer.map(_.toInt) ~ sizeSemantics.? ~ ")")
      ...
      | (StringInIgnoreCase("CHAR", "CHARACTER") ~ "(" ~ integer.map(_.toInt) ~ sizeSemantics.? ~ ")")
...
object OracleSQL extends SQLBaseObject {
  ...
  /** Parse all statements in the input, attaching source comments to CREATE TABLE statements. For CREATE TABLE
    * statements, comments immediately before the CREATE keyword are attached to the table, and comments immediately
    * before each column definition are attached to that column.
    *
    * This is the library-level equivalent of the CLI's --with-comments flag.
    *
    * @param input
    *   SQL source containing one or more statements
    * @return
    *   Sequence of StatementParseResult with sourceComment populated for CREATE TABLE statements
    */
  def parseAllWithComments(input: String): Seq[sequala.common.statement.StatementParseResult] = {
    import sequala.common.statement.StatementParseResult
    import sequala.schema.{CreateTable, Statement, Unparseable}

    // Helper to find where preceding comments start before a statement position
    def findPrecedingCommentStart(content: String, statementPos: Int): Int = {
    ...
    // Parse all statements using the regular parser
    val results = parseAll(input)

    // Post-process: for CreateTable statements, re-parse with comment attachment
    results.map { parseResult =>
      parseResult.result match {
        case Right(stmt) =>
          stmt match {
            case ct: CreateTable[?, ?, ?] =>
              // Get source text for this statement, including preceding comments
              val extendedStartPos = findPrecedingCommentStart(input, parseResult.startPos)
              val sourceTextWithComments =
                input.substring(extendedStartPos, math.min(parseResult.endPos, input.length))

              // Re-parse with comment attachment
              parseCreateTableWithComments(sourceTextWithComments) match {
                case Right(ctWithComments) =>
                  StatementParseResult(Right(ctWithComments), parseResult.startPos, parseResult.endPos)
                case Left(_) =>
                  // Fall back to original if re-parse fails
                  parseResult
              }
            case _ => parseResult
          }
        case Left(_) => parseResult
      }
    }
  }
}
