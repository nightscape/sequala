...
import sequala.common.expression.{Column, Expression, LongPrimitive, StringPrimitive}
...
class SQLParserSpec extends Specification {
  ...
  implicit def StringToName(s: String): sequala.common.Name = sequala.common.Name(s)
...
SELECT A, B FROM R;""") { q =>
    ...
    "Parse queries with IN expressions" >> {
      testSelect("SELECT `_c1` FROM R WHERE `R`.`_c0` IN ('2','3','4');") { q =>
        q.where.get should beAnInstanceOf[sequala.common.expression.InExpression]
      }
  ...
  "The Statement Parser" should {
    ...
    "parse CREATE TABLE statements" >> {
      ...
      statement[CreateTableStatement]("""
        ...
        stmt.schemaTable.table.name must beEqualTo("foo")
        stmt.schemaTable.orReplace must beTrue
        stmt.schemaTable.table.columns.map(_.name) must contain(exactly("bar", "baz", "broz"))
        stmt.schemaTable.table.columns.find(_.name == "broz").get.nullable must beFalse
        stmt.schemaTable.table.primaryKey must beSome
        stmt.schemaTable.table.primaryKey.get.columns must contain("broz")
        stmt.schemaTable.table.indexes.length must beEqualTo(2)
      }
    }
