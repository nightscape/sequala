...
| `dump-filter-ddl.jq` | Filter _AT/_HT from dump, group by schema | `dump` |
| `dump-filter-data.jq` | Transform XMDM_CONF rows to YAML | `dump` |
| `unified-dump-filter.jq` | Combined DDL+metadata transform with overrides | `dump-unified` (recommended) |
