...
```bash
./sql/oracle/oracle-comparison.sh dump "GUI_XMDM%" "GUI_XMDM.XMDM_CONF%" 1521 dump \
  sql/oracle/dump-filter-ddl.jq sql/oracle/dump-filter-data.jq
```

This creates:
- `{schema}-PROD-tables.sql` - DDL for base tables (excluding _AT/_HT)
- `{schema}-PROD-tables.yaml` - Metadata from XMDM_CONF_TABLE/COLUMN
