...
The Taskfile-based pipeline compares the state of `oracle-orig` (populated from migration scripts) with the desired state (derived from the same scripts). In theory, this should produce minimal differences. In practice, ~1361 migration statements are generated.
...
All tasks are run from the `sql/oracle/` directory using [Task](https://taskfile.dev):

```bash
cd sql/oracle
task --list              # Show available tasks
task container:status    # Check container status
```
...
- [ ] **Re-run full pipeline after fixes**
  - Stop and restart containers: `task container:stop && task container:start`
  - Run migrate-orig: `task migrate:orig`
  - Generate prod tables: `task generate:prod-tables FORCE=true`
  - Compare: `task compare:desired-state OUTPUT_DIR=/tmp/migrations-fixed`
...
```bash
task container:start-orig   # Start oracle-orig on port 1521
# OR
task container:start        # Start both oracle-orig (1521) and oracle-new (1522)
```

### Step 2: Run Migrations

The `migrate:orig` task handles schema creation automatically via `_setup.sql`:

```bash
task migrate:orig   # Parse SQL files, create schemas, and apply migrations
```
...
task compare:desired-state \
  CONN='system/orakel@//localhost:1521/FREEPDB1' \
  OUTPUT_DIR=migrations-against-orig 2>&1 | tee /tmp/compare-orig.log
```
...
| File | Purpose |
|------|---------|
| `Taskfile.yml` | Main orchestration (replaces oracle-comparison.sh) |
| `ddl-to-derived-tables.jq` | Generates _AT and _HT tables from base tables |
| `ddl-to-metadata.jq` | Extracts metadata annotations as YAML |
| `ddl-to-sync.jq` | Generates metadata INSERT/UPDATE statements |
| `merge-ddl-and-metadata.jq` | Creates DESIRED_STATE files from DDL + metadata |
| `unified-dump-filter.jq` | Filters dump output |
| `compare.sql` | SQL*Plus script for database comparison via db link |

### Key Taskfile Commands

| Task | Description |
|------|-------------|
| `task container:start` | Start both oracle-orig (1521) and oracle-new (1522) containers |
| `task container:stop` | Stop and remove all containers |
| `task container:status` | Show container status |
| `task migrate:orig` | Parse SQL files and run migrations on oracle-orig |
| `task migrate:new` | Process DESIRED_STATE files and run on oracle-new |
| `task generate:prod-tables` | Generate *-PROD-tables.sql from schema directories |
| `task compare:desired-state` | Generate migrations from DESIRED_STATE vs database |
| `task dump:unified` | Export DDL and data using unified filter |
| `task sync` | Generate combined schema+data migrations |
| `task roundtrip` | Full end-to-end roundtrip test |
| `task roundtrip:status` | Show which roundtrip steps are completed |
| `task roundtrip:clean` | Clean roundtrip state to start fresh |
...
```bash
task dump:unified \
  CONN='system/pass@//localhost:1521/FREEPDB1' \
  DDLS_PATTERN='GUI_XMDM%' \
  DATA_PATTERN='GUI_XMDM.XMDM_CONF%'
```
...
# Full roundtrip test (dump → apply to new container → compare)
task roundtrip \
  CONN='system/orakel@//localhost:1521/FREEPDB1' \
  DDLS_PATTERN='GUI_XMDM%' \
  DATA_PATTERN='GUI_XMDM.XMDM_CONF%'

# Check roundtrip status
task roundtrip:status
```
